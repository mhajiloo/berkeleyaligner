...
import edu.berkeley.nlp.util.IteratorIterator;
...
public class SentencePairReader {
	...
	public class PairDepot implements Iterable<SentencePair> {
		...
		public Iterator<SentencePair> iterator() {
			...
			iter = Iterators.maxLengthIterator(iter, maxPairs);
			...
			for (int i = 0; i < offset; i++) {
				if (iter.hasNext()) {
					...
					LogInfo.error("Pairs available (%d) less than offset (%d)", i, offset);
					break;
				}
			}
	...
	public class PairIterator implements Iterator<SentencePair> {
		...
		private Iterator<Tree<String>> englishTrees;
		private Iterator<Tree<String>> foreignTrees;
		...
		private void openOutputAndRejectFiles(String baseFileName) {
			if (saveInput) {
				...
				englishTagOutput = IOUtils.openOutHard(fileName + "." + englishTagExtension);
				foreignTagOutput = IOUtils.openOutHard(fileName + "." + foreignTagExtension);
				englishTreeOutput = IOUtils.openOutHard(fileName + "." + englishTreeExtension);
				foreignTreeOutput = IOUtils.openOutHard(fileName + "." + foreignTreeExtension);
				alignmentOutput = IOUtils.openOutHard(fileName + "." + wordAlignmentExtension);
			}
			if (saveRejects) {
				...
				englishTagRejects = IOUtils.openOutHard(fileName + "." + englishTagExtension);
				foreignTagRejects = IOUtils.openOutHard(fileName + "." + foreignTagExtension);
				englishTreeRejects = IOUtils.openOutHard(fileName + "." + englishTreeExtension);
				foreignTreeRejects = IOUtils.openOutHard(fileName + "." + foreignTreeExtension);
				alignmentRejects = IOUtils.openOutHard(fileName + "." + wordAlignmentExtension);
			}
		}
		...
		private void readNextAlignment(BufferedReader in, SentencePair pair) {
			if (in == null) return;
			try {
				if (in.ready()) {
					Alignment alignment = new Alignment(pair);
					alignment.parseAlignments(in.readLine(), reverseAlignments);
					pair.setAlignment(alignment);
				}
			} catch (IOException e) {
				LogInfo.error("Problem reading alignment file: " + e.getLocalizedMessage());
			}
		}
		...
		private Iterator<Tree<String>> getTreeIterator(String treeFileName) {
			...
			try {
				...
				//			LogInfo.logs("No parses loaded from " + treeFileName);
			}

			if (treeReader == null) {
				return new ArrayList<Tree<String>>().iterator();
			} else {
				return treeReader;
			}
		}
		...
		private void loadNext() {
			try {
				...
				while (next == null && englishIn.ready() && frenchIn.ready()) {
					...
					readNextAlignment(alIn, pair);
					if (englishTrees.hasNext()) pair.setEnglishTree(englishTrees.next());
					if (foreignTrees.hasNext()) pair.setForeignTree(foreignTrees.next());
		...
		public void skipNext() {
			...
			englishTrees.next();
			foreignTrees.next();
			readNextAlignment(alIn, new SentencePair(1, null, null, null));
		}
	...
	// TODO Support tags in sentence pairs
	...
	private void readSentencePairsUsingList(List<String> baseFileNames, int offset,
			int maxSentencePairs, List<SentencePair> sentencePairs, Filter<SentencePair> filter) {
		initSaveDirectories();
